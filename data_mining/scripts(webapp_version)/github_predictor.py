...
def main():
    ...
    suggested_language = top_languages[classifier.predict([100, 0, 100])-1]

    print "coefficients of each weight = " + str(classifier.coef_)

    # print training mean accuracy using 'score'
    mean_acc = classifier.score(training_features, training_labels)
    ...
    with open('../../src/main/resources/static/language.csv', 'w') as f:
        w = csv.writer(f)
        w.writerow(['language', 'score'])
        w.writerow([suggested_language, int(mean_acc*100)])
